   ...
   /* Initialise the best distance,deploy time and arrive time. */
    ...
    for(i ; i<num_assets; i++){
        ...
        if(assets[i].type==type) {
           ...
           int is_bussy;
           is_bussy = time_difference(assets[i].t_bussy_until,md_call.the_time);
            if( is_bussy > 0 ) {
                continue;
            }
                          /*For each asset*/ 
...
/*print the result*/
time bussy_until=time_addition(md_call.the_time,best_deploy_time);

best_match->t_bussy_until=bussy_until;
