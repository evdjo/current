...
#define MIL 1000000.0
    ...
    /* Data structure to store time, latitude and longitude of a location*/
    ...
    /* node used for the linked list, where the good locations are stored*/
    typedef struct node {
        loc_t loc;
        struct node * next;
    } node_t;
    ...
    /*Loops through the linked list and writes to the the output file */
    ...
    /* Tries to synchronise the two streams, so that both have the same time*/
    int sync_time_gps(FILE * file1, FILE * file2,
            stream_t * strm_1, stream_t * strm_2);
    /* Tries to synchronise the two streams so that both have good satellite fix*/
    int sync_satelites(FILE * file1, FILE * file2,
            stream_t * strm_1, stream_t * strm_2);

    /* Reads a line from a stream , returns what kind of sentence was read*/
    ...
    /* turn degrees to a decimal representation , and put it to the loc pointer passed */
